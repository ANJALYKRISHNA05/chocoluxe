...
exports.addToWishlist = async (req, res) => {
  try {
    ...
    const itemExists = wishlist.items.some((item) => item.product.toString() === productId && item.sku === variant.sku)
    if (itemExists) {
      return res.status(400).json({
        success: false,
    ...
    if (wishlist && wishlist.items.length > 0) {
      wishlistItems = wishlist.items.map((item) => {
        ...
        if (variant && !errorMessage) {
          ...
          basePrice =
            variant.salePrice && variant.salePrice < variant.regularPrice && variant.salePrice > 0
              ? variant.salePrice
